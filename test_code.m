...
if ~exist('DREAMER','var')
    load("C:\Users\Mukil Saravanan\Downloads\MatlabCode\ECG_sample_dataset.mat");
...
total_person=length(Data);
total_stimuli=length(Data.ECG.stimuli);
total_baseline=length(Data.ECG.baseline);
total_channel=size(Data.ECG.stimuli{1,1},2);
...
for stimulus=1:total_stimuli
    for channel=1:total_channel
        temp_obj=Feature_extracter(Data.ECG.stimuli{stimulus,1}(end-samples_per_min+1:end,channel),Fs);
        temp_obj.init();
...
for baseline=1:total_baseline
    for channel=1:total_channel
        temp_obj=Feature_extracter(Data.ECG.baseline{baseline,1}(end-samples_per_min+1:end,channel),Fs);
        temp_obj.init();
